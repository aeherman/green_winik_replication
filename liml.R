library(Matrix)
liml <- function(Y,D,Z,X,intercept=TRUE,
  ...
  return(ivmodelObject$LIML)
}


#### liml2 ####
liml2 <- function(Y,D,Z,X,intercept=TRUE,
                 beta0=0,alpha=0.05,k=c(0,1),
                 manyweakSE = FALSE, heteroSE = FALSE, clusterID = NULL,
                 deltarange = NULL, na.action = na.omit) {
  ...
  if( (#!is.vector(D) &&
      ...
      #(is.matrix(D) && ncol(D) != 1) || #(is.data.frame(D) && ncol(D) != 1)
      (!is.numeric(D))) stop("D is not a numeric vector.")
  Y = as.numeric(Y)#; D = as.numeric(D)
  if(length(Y) != nrow(D)) stop("Dimension of Y and D are not the same!")
  ...
  # Fit adjustment model
  ff = terms(Y ~ . -1,data=allDataOrig) # remove specification of D
  mf = model.frame(ff,allDataOrig)
  
  # Declare Y and D
  Y = as.matrix(mf$Y); colnames(Y) = "Y"
  D = as.matrix(mf[grep("D.*", colnames(mf))])
